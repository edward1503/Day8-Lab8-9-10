...
| Rule 7 (BOM/invisible char) | Baseline sample không có BOM → không kích hoạt | Inject `t7a` (BOM `\ufeff`) + `t7b` (zero-width space `\u200b`) vào `inject_test_rules.csv` → `quarantine_records +2`, reason=`bom_or_invisible_char_in_text` | `artifacts/quarantine/quarantine_inject-rules-test.csv` |
| Rule 8 (future date cutoff) | Baseline sample không có effective_date > 2030 | Inject `t8` (`effective_date=2099-01-01`) → `quarantine_records +1`, reason=`effective_date_beyond_future_cutoff` | `artifacts/quarantine/quarantine_inject-rules-test.csv` |
| Rule 9 (whitespace normalization) | Baseline sample không có tab/multi-space | Inject `t9` (tab + multi-space) → `cleaned_records +1` với marker `[cleaned: whitespace_normalized]`; E9 FAIL(warn) với `short_chunks=1` cho `t_e9` (3 từ < 5) | `artifacts/cleaned/cleaned_inject-rules-test.csv`, `artifacts/logs/run_inject-rules-test.log` |
| E7 no_bom_or_invisible (halt) | Bypass Rule 7 → BOM lọt vào cleaned | E7 là "lưới an toàn thứ hai" — nếu cleaning bypass thì E7 halt pipeline | `contracts/data_contract.yaml` mục E7 |
...
- Inject scenario cho Rule 7/8/9 đã được chứng minh qua `run_id=inject-rules-test` với artifact `quarantine_inject-rules-test.csv` (BOM × 2, future date × 1) và log `run_inject-rules-test.log`.
