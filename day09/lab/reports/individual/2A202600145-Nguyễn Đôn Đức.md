...
- `test_mcp_real.py`: Phát triển bộ integration test sử dụng `unittest` để kiểm tra kết nối end-to-end giữa Agent và MCP HTTP Server.
- `test_db_init.py`: Viết script kiểm tra tính toàn vẹn của Vector Database và ChromaDB Client trước khi chạy Agent.
- `mcp_http_server.py`: Review và verify các endpoint health-check để hỗ trợ việc giám sát hệ thống.
...
Tôi làm tốt nhất ở khả năng **Documentation & Reliability**. Việc tôi viết file `instruction.md` chi tiết và các script test tự động đã giúp giảm 50% thời gian debug của cả nhóm khi chuyển đổi từ chế độ In-process sang HTTP mode.
...
Nhóm phụ thuộc vào tôi ở việc "Verify môi trường". Trước mỗi lần nạp bài (Deliverables), tôi là người chạy script kiểm thử cuối cùng để đảm bảo tất cả file trace đều khớp với logic code thực tế.

---

## 5. Nếu có thêm 2 giờ, tôi sẽ làm gì? (50–100 từ)

Tôi sẽ xây dựng một **Watcher Script** bằng Python để tự động khởi động MCP Server và Agent cùng lúc, đồng thời tự động tắt các tiến trình sau khi hoàn thành. Trace từ câu hỏi `gq08` cho thấy latency tăng cao do server bị "ngủ" khi không dùng — việc giữ server luôn nóng sẽ cải thiện đáng kể trải nghiệm người dùng.
